    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['smart:complain:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['smart:complain:remove']"
        >删除</el-button>
      </el-col>
      <!-- <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['smart:complain:export']"
        >导出</el-button> -->
      </el-col>
...
<script>
import { listComplain, getComplain, delComplain, addComplain, updateComplain,changeStatus } from "@/api/smart/complain";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除投诉信息编号为"' + complainIds + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/complain/export', {
        ...this.queryParams
    ...
    handleStatusChange(row) {
      ...
      this.$modal.confirm('确认要"' + text + '"吗？').then(function() {
        return changeStatus({complainId:row.complainId, status:row.status});
      }).then(() => {
    ...
    handleProcess(row){
      this.$modal.confirm('确认要处理吗？').then(function() {
        return changeStatus({complainId:row.complainId, status:'1'});
      }).then(() => {
