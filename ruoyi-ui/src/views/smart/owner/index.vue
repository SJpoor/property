        ...
        <pane size="84">
          <el-col>
            ...
            >
              ...
              <el-table-column label="业主姓名" align="center" key="nickName" prop="nickName" v-if="columns[2].visible" :show-overflow-tooltip="true" />
              <el-table-column label="业主性别" align="center"  prop="sex" >
                <template slot-scope="scope">
                  <dict-tag :options="dict.type.sys_user_sex" :value="scope.row.sex"/>
                </template>
              </el-table-column>
              <el-table-column label="手机号" align="center" key="phonenumber" prop="phonenumber" v-if="columns[4].visible" width="110" />
...
<!--              <el-table-column label="状态" align="center" key="status" v-if="columns[5].visible" width="80">
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-row>
          <el-col :span="12">
            <el-form-item v-if="form.userId == undefined" label="业主账号" prop="userName">
              <el-input v-model="form.userName" placeholder="请输入业主账号" maxlength="30" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item v-if="form.userId == undefined" label="业主姓名" prop="nickName">
              <el-input v-model="form.nickName" placeholder="请输入业主姓名" maxlength="30" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
<!--            通过v-if判断添加或修改显示状态，当usirid = undefined是该框会显示-->
...
<!--              绑定到form.roleIds-->
    ...
    <el-dialog :title="title" :visible.sync="assignHouseOpen" width="500px" append-to-body>
      <el-form ref="assignHouseForm" :model="assignHouseForm" :rules="rulesHouse" label-width="80px">
        <el-form-item label="房屋" prop="houseIds">
          <el-select v-model="assignHouseForm.houseIds" placeholder="请选择房屋" :multiple="true" clearable style="width: 100%">
    ...
    <el-dialog :title="title" :visible.sync="assignSpaceOpen" width="500px" append-to-body>
      <el-form ref="assignSpaceForm" :model="assignSpaceForm" :rules="rulesSpace" label-width="80px">
        <el-form-item label="车位" prop="parkingSpaceIds">
          <el-select v-model="assignSpaceForm.parkingSpaceIds" placeholder="请选择车位" :multiple="true" filterable clearable style="width: 100%">
  ...
  methods: {
    ...
    handleAdd() {
      ...
      getUser().then(response => {
        ...
        this.roleOptions = response.roles;//获取角色数据
        // this.form.roleOptions = '11';
        this.open = true;
        this.title = "添加用户";
    ...
    handleAssignHouse(row){
      ...
      this.assignHouseForm.ownerId = row.userId
      this.title = "分配房屋"; */
      const ownerId = row.userId;
      this.$router.push("/smart/owner-auth/house/" + ownerId);
    },
    // 分配车位
    handleAssignSpace(row){
      /* this.resetSpaceForm();
      this.assignSpaceOpen = true;
      this.assignSpaceForm.ownerId = row.userId
      this.title = "分配车位"; */
      const ownerId = row.userId;
      this.$router.push("/smart/owner-auth/parkingSpace/" + ownerId);
    },
    // 退房
    handleReturnHouse(row){
      this.$modal.confirm('确认要退房吗？').then(function() {
        return deleteOwnerHouse({ownerId:row.userId, houseId:row.house.houseId});
      }).then(() => {
        this.getList();
    ...
    handleReturnSpace(row){
      this.$modal.confirm('确认要退车位吗？').then(function() {
        return deleteOwnerParkingSpace({ownerId:row.userId, parkingSpaceId:row.parkingSpace.parkingSpaceId});
      }).then(() => {
        this.getList();
