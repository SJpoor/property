...
<!--  车位租赁-->
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="车位" prop="ownerId">
    ...
    <el-table v-loading="loading" :data="rentList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="费用名称" align="center" prop="fee.feeName" width="110"/>
      ...
      <el-table-column label="是否生成费用" align="center" prop="endTime" width="110">
        <template slot-scope="scope">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="业主" prop="ownerId">
          <el-select v-model="form.ownerId" placeholder="请选择业主" filterable clearable style="width: 100%" @change="handleOwnerChange">
...
<script>
import { listRent, getRent, delRent, addRent, updateRent,changeStatus,addRentFee } from "@/api/smart/parkingRent";
...
export default {
  ...
  methods: {
    ...
    getFeeList() {
      listFee({pageNum: 1,pageSize: 1000}).then(response => {
        ...
        if(this.feeList.length > 0){
          const feeObj = this.feeList.find(item => item.feeType == '4');
          this.feeList = []
          this.feeList.push(feeObj)
          console.info("feeObj.feeId",feeObj.feeId)
          this.form.feeId = feeObj.feeId
          console.info("this.form.feeId",this.form.feeId)
        }
      });
    },
    /** 查询用户列表 */
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除车位租赁信息？').then(function() {
    ...
    handleOwnerChange(v){
      this.getPrkingSpaceList(v)
    },
    handleFee(row){
      this.$modal.confirm('是否确认要生成费用？').then(function() {
        return addRentFee(row);
      }).then(() => {
        this.getList();
